...
| :--- | :--- | :--- |
| **All-In-One** | `sh deployment.sh all-in-one` | Runs the entire preparation and deployment stack. |
| **Install** | `sh deployment.sh install-stack` | Installs Python venv and Kolla config without running deployment. |
| **Deploy** | `sh deployment.sh deploy` | Runs just the Kolla-Ansible deployment phase. |
| **Remove** | `sh deployment.sh remove` | Uninstalls and safely cleans up all OpenStack containers, networks, and configuration files. |
| **Status** | `sh deployment.sh status` | Displays the status of your tools and network interfaces. |
| **Tunnel** | `sh tunnel.sh` | Interactive script to bind Horizon to a public domain via Cloudflare. |
