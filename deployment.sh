...
install_kolla_stack() {
    ...
    "$VENV_PATH/bin/kolla-ansible" install-deps

    echo "Generating Kolla passwords..."
    "$VENV_PATH/bin/kolla-genpwd"
...
run_deploy() {
    ...
    "$VENV_PATH/bin/kolla-ansible" -i ./all-in-one bootstrap-servers

    echo "Running prechecks..."
    "$VENV_PATH/bin/kolla-ansible" -i ./all-in-one prechecks

    echo "Executing deployment (this may take 20-40 minutes)..."
    "$VENV_PATH/bin/kolla-ansible" -i ./all-in-one deploy

    echo "Installing OpenStack CLI client..."
    "$VENV_PATH/bin/pip" install python-openstackclient -c https://releases.openstack.org/constraints/upper/2025.2

    echo "Running post-deploy configuration..."
    "$VENV_PATH/bin/kolla-ansible" -i ./all-in-one post-deploy
